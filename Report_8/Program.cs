    ...
    public class Buffering
    {
        ...
        public void SendData(AdlsClient c, int i,BlockingCollection<int> b,string filename, string path)
        {
            ...
            while (length>0)
            {
                ...
                //sending data via 2 MB buffer if data is more than 1 MB
                else
                {
                    //sending data via 2 MB buffer
                    b.Add(1);
                    if (buffer2[i] == null)
                    { 
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Parallel.For(0, 20, i => {
                    ...
                    if (i < 10)
                    {
                        path[i] = @"C:\Users\kchah\OneDrive\Desktop\ADLS\Report_8\Input\1MB\" + (i + 1) + ".txt";
                    }
                    else
                    {
                        path[i] = @"C:\Users\kchah\OneDrive\Desktop\ADLS\Report_8\Input\2MB\" + ((i%10) + 1) + ".txt";
                    }
